# --- Pacman Game (Single Agent with DFS & BFS) ---
import pygame
import time
...
SEARCH_ALGO = 'bfs'  # Change to 'bfs' or 'dfs' to switch search strategy
...
class Game:
    def __init__(self):
        ...
        pygame.display.set_caption("Pacman - DFS & BFS")
        ...
        # Choose and compute path
        if SEARCH_ALGO == 'dfs':
            self.pacman.path = dfs(self.maze, self.start_pos, self.goal_pos)
        elif SEARCH_ALGO == 'bfs':
            self.pacman.path = bfs(self.maze, self.start_pos, self.goal_pos)
        else:
            print(f"Unknown search algorithm: {SEARCH_ALGO}")

    def draw(self):
        self.screen.fill(BLACK)
        self.maze.draw(self.screen, self.goal_pos)
